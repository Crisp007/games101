...
static bool insideTriangleMsaa(float x_center, float y_center, const Vector4f* _v)
{   
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
        ...
        for (int i = (int)min_x; i <= (int)max_x; i++) {
            for (int j = (int)min_y; j <= (int)max_y; j++) {
                if (insideTriangle(i, j, v)) {
                    auto[alpha, beta, gamma] = computeBarycentric2D(float(i), float(j), v);
                    float Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w()); //w negative or positive?
                    float z_interpolated = Z*(alpha*v[0].z()/v[0].w() + beta*v[1].z()/v[1].w() + gamma*v[2].z()/v[2].w());
                    z_interpolated *= -1.0f;
                    int index = j*width+i;
    ...
    // mass superSampling
        ...
        for (int i = (int)min_x; i <= (int)max_x; i++) {
            for (int j = (int)min_y; j <= (int)max_y; j++) {
                Vector3f average_pixel_color(0, 0, 0);
                    ...
                    for (int n = 0; n < std::sqrt(msaa); n++) {
                        ...
                        if (insideTriangleMsaa(x, y, t.v)) {
                            ...
                            if (z_interpolated < msaa_depth_buf[index]) {
                                ...
                                msaa_frame_buf[index] = pixel_color;
                            }
                        }
                        average_pixel_color += msaa_frame_buf[index];
                    }
                }
                if (covered_sample != 0) {
                    float covered_ratio = (float)covered_sample/msaa;
                    // if (covered_ratio != 1.0f) {
                    //     outFile << "x:" << i << " y: " << 700-j << "  covered_sample: " << covered_sample << std::endl; 
                    //     outFile << "average_color/covered_sample: " << average_pixel/covered_sample << "  covered_ratio: " << covered_ratio << std::endl;
                    //     outFile << "t.v[0]: " << v[0](0) << " " << (700-v[0][1]) <<
                    //         " t.v[1]: " << v[1](0) << " " << (700-v[1][1]) <<
                    //         " t.v[2]: " << v[2](0) << " " << (700-v[2][1]) << std::endl;
                    // }
                
                    Vector3f point(float(i), float(j), -1/*no use, random number*/);

                    set_msaa_pixel(point, average_pixel_color/msaa);
                    // set_msaa_pixel(point, t.getColor(), covered_ratio);
                }
