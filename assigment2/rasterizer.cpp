...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
            ...
            for (int j = (int)min_y; j <= (int)max_y; j++) {
                if (insideTriangle(i, j, t.v)) {
                    ...
                    if (z_interpolated < depth_buf[index]) {
                        // if (debug_flag) {
                        //     debug_flag = false;
                        //     std::cout << "this t: " << t.v[0] << std::endl;
                        //     std::cout << "this t z_interpolated: " << z_interpolated << std::endl;
                        // }
    ...
    // mass superSampling
        ...
        //1/sqrt(msaa)
        // float gap = 1.0f/(sqrt(msaa)+1.0f);
        // for (int i = (int)min_x; i <= (int)max_x; i++) {
        //     for (int j = (int)min_y; j <= (int)max_y; j++) {
        //         int covered_sample = 0;
        //         for (int m = 1; m <= std::sqrt(msaa); m++)  {
        //             for (int n = 1; n <= std::sqrt(msaa); n++) {
        //                 float x = float(i)+m*gap;
        //                 float y = float(j)+n*gap;
        ...
        //             // if (covered_ratio != 1) {
        //             //     std::cout << "x:" << i << " y: " << j << "  covered_sample: " << covered_sample << std::endl; 
        //             //     std::cout << "color: " << t.getColor() << "  covered_ratio: " << covered_ratio << std::endl;
        //             //     std::cout << "t.v[0]: " << t.v[0] << std::endl;
        //             // }
        //             if (covered_ratio != 1) {
...
void rst::rasterizer::set_msaa_pixel(const Eigen::Vector3f& point, const Eigen::Vector3f& color, float covered_ratio)
{
    //old index: auto ind = point.y() + point.x() * width;
    auto ind = (height-1-point.y())*width + point.x();
    frame_buf[ind] = frame_buf[ind]*(1.0f-covered_ratio) + color*covered_ratio;
